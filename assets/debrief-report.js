/* ============================================================
 * LMAeroFit · assets/debrief-report.js
 * RENDERIZADOR do Relatório Pós-Prova (gráficos SVG + mapa Leaflet/OpenTopoMap).
 * Consome o objeto devolvido por LMA.debrief.analyze (debrief-core.js).
 *
 * API:  LMA.debrief.renderToHTML(report) -> string
 *       LMA.debrief.renderReport(report, containerEl)  (inicializa o mapa)
 *
 * O mapa usa Leaflet (window.L) + tiles OpenTopoMap, carregados no HTML da
 * ferramenta. Sem Leaflet, cai num traço SVG simples (fallback).
 * ============================================================*/
(function (root) {
  'use strict';
  ...
  // Fallback SVG (sem Leaflet) + inicialização do mapa Leaflet
  function initRouteMap(r, tries) {
    tries = tries || 0;
    const el = (typeof document !== 'undefined') && document.getElementById('dbf-map');
    if (!el || !r.series.has.gps) return;
    const s = r.series;
    const pts = [];
    for (let i = 0; i < s.N; i++) if (s.lat[i] != null && s.lng[i] != null) pts.push([s.lat[i], s.lng[i], i]);
    if (pts.length < 5) return;

    if (typeof window === 'undefined' || !window.L) {
      if (tries < 20) { setTimeout(() => initRouteMap(r, tries + 1), 250); return; }
      return drawSvgFallback(el, r, pts); // Leaflet não carregou
    }
    const L = window.L;
  ...
  function drawSvgFallback(el, r, pts) {
  ...
  function chartZones(r) {
    ...
    z.forEach(x => {
      h += `<div style="display:grid;grid-template-columns:104px 1fr 78px;gap:8px;align-items:center;font-family:var(--mono);font-size:11px">
        ...
        <div style="background:var(--bg-0);height:14px;border-radius:3px;overflow:hidden"><div style="width:${(x.pct / maxPct * 100).toFixed(0)}%;height:100%;background:${colors[x.z] || HEX.amber}"></div></div>
    ...
    if (r.splits && r.splits.length > 1) {
      ...
      sp.forEach(x => {
        ...
        h += `<div style="display:grid;grid-template-columns:54px 1fr 120px;gap:8px;align-items:center;font-family:var(--mono);font-size:10.5px">
          ...
          <div style="background:var(--bg-0);height:11px;border-radius:2px;overflow:hidden"><div style="width:${(val / maxV * 100).toFixed(0)}%;height:100%;background:${HEX.amber}"></div></div>
